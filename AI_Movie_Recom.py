...
def dots():
    for _ in range(3): print(Fore.YELLOW+".", end="", flush=True);
time.sleep(0.5)
...
def show(recs, name):
    print(Fore.YELLOW+f"\n✏️ AI_Analyzed Movie Recommendations for {name}:")
...
def get_genre():
    ...
    while True:
        ...
        if x.isdidgit() and 1 <= int(x) <= len(genres): return genres[int(x)-1]
        x=x.title()
...
genre=get_genre()
mood=input(Fore.YELLOW+"How do yo feel today? (Describe your mood) : ").strip()
print(Fore.BLUE+"\nAnalyzing Mood", end="", flush=True); dots()
mp=TextBlob(mood).sentiment.polarity
md="postive 😊"if mp>0 else "negative 😔"if mp<0 else"neutral 😐"